...
<DiagramFrame
  ...
  size="wide"
  narration="A model card is a spec sheet, not a scorecard. Price, modality, context versus recall, and instruction following are each a single published measurement taken under its own conditions, price at listed rates, recall at a given context length, instruction following on one benchmark. Strength in one predicts nothing about another, and a field with no published result for your workload is simply unknown. Read across them and you get a profile: where a model fits and where it falls short. That profile picks a candidate for a workload, it never ranks a winner."
  caption={
    <>
...
<MarketingDecoderInteractive narration="Launch claims hide behavior. One model is announced as four times faster, which turns out to mean more tokens per second, faster feedback on screen rather than faster completion of your task. Another claims long-context consistency, yet measured recall falls from 78% to 32% as the input grows. Translating a claim means asking what actually changed and what it costs. Four times faster, 78% to 32% recall, different numbers, same move: translate before you evaluate, because the marketing line and the engineering trade-off are never the same statement." />
