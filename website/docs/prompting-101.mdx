...
<PromptComparison
  narration="Compare an ineffective prompt with an effective one. The ineffective prompt just asks, can you help us think through usage-based billing? Thanks. The effective prompt opens by naming the artifact: produce an implementation plan for migrating workspace billing from seat-based pricing to usage-based pricing. It then does three things. Target: it names the artifact, audience, and business change up front. Scope: it defines the systems and risks the plan must cover. Contract: it specifies the sections a reviewer can evaluate quickly."
  goodNotes={[
