...
<DiagramFrame kicker="Agent mechanics" title="The context window is a living stream" size="wide" narration="The model never remembers the last step. The harness rebuilds the window every turn and decides what enters and which role carries it: system instructions, tool schemas, previous turns, memory, a tool call, the result it returned, your current request. Those are not interchangeable prose. A tool definition is an action contract; a tool result is evidence returned by the environment. And none of it changes behavior until the harness serializes it back into the next request, which is why a failed test or a project rule only matters once a later call carries it." caption="Each turn rebuilds the context window. The layer view is only a source map; tool calls and tool results matter after the harness serializes them into raw context for a later request.">
