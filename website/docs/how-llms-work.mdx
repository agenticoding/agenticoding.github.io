...
A publisher document is useful only when it changes a workload decision. Consider a text-only agent that turns one day of email into a prioritized todo list: it must recover commitments, normalize dates, merge duplicates, rank work, and emit a valid structure. Read one published model card at a time across cost, published modality, recall, and hostile-input scope. A result in one field is only a candidate for the next decision—not a winner for the whole route.

<DiagramFrame
  kicker="Model selection"
  title="Read published fields against one workload"
  size="wide"
  caption={
    <>
      Each card stays within its published source boundary. Its icon and title
      name the field; its score indicator signals the narrow published result:
      strong, mixed, limited, or unknown. No card is an overall winner.
    </>
  }
>
