...
<DiagramFrame kicker="Agent mechanics" title="Agents run inside a harness loop" size="standard" narration="An agent isn't a thinking cycle, it's an ordinary application loop wrapped around repeated model calls. Each pass assembles context, asks the model for the next output, validates that output, runs any approved action, and folds the result back into the next context. That's the whole thing. The model only ever predicts tokens; everything that makes it useful, checking the output against schema and budget and permissions, executing tools, capturing observations, deciding when to stop, lives in the harness around it. Read the boundary that way and the whole thing stops being mysterious: it's a deterministic control plane steering a probabilistic engine." caption={<>
...
```ts narration="The loop keeps a fixed instruction prefix and an append-only transcript. Each turn rebuilds context from that prefix plus everything accumulated, asks the model, inspects the raw reply, and appends it. Tool calls run outside the model, their observations are appended, and the harness decides whether to stop on completion, an exhausted budget, or a full context window."
