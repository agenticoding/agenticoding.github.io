...
export const SEQ_ROWS = [
  ...
  { type: 'note',    on: 'agent', text: 'Decides to run tests via Bash' },
...
{/* earlyStart: figure may be partially visible on load, so animation phase starts earlier.
    skipReveal: child component manages its own opacity; no container fade-in needed. */}
<ScrollDrivenFigure phaseEnd={0.585} earlyStart skipReveal caption="Perceive, reason, act, observe, verify — iterate until done.">
  <AgentLoopDiagram />
</ScrollDrivenFigure>
...
{/* phaseEnd: scroll 0.7 viewport-heights past entry trigger to animate all 14 sequence rows */}
<ScrollDrivenFigure phaseEnd={0.7} earlyStart skipReveal caption="Everything is text flowing through a context window.">
  <SequenceDiagram
...
<ScrollDrivenFigure phaseEnd={0.6} earlyStart skipReveal caption="Same code, same model — different context produces a different solution class.">
  <ContextBiasDiagram />
</ScrollDrivenFigure>
...
:::tip Preview: [Lesson 7 — Planning & Execution](../practical-techniques/lesson-7-planning-execution.md)
