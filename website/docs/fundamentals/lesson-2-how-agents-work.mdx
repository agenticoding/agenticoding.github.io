...
- <InlineEmojiImage asset={EMOJI.compass} size={18} /> **System or developer instructions:** durable rules, behavior constraints, safety policy, and output format.
- <InlineEmojiImage asset={EMOJI.plug} size={18} /> **Tool definitions:** names, descriptions, and schemas for actions the model may request.
- <InlineEmojiImage asset={EMOJI.chat} size={18} /> **Conversation history:** prior user and assistant messages replayed as context.
- <InlineEmojiImage asset={EMOJI.tools} size={18} /> **Assistant tool calls:** structured action requests the model emitted on earlier turns.
- <InlineEmojiImage asset={EMOJI.receipt} size={18} /> **Tool observations:** command output, file contents, search results, test failures, and API responses.
- <InlineEmojiImage asset={EMOJI.database} size={18} /> **Retrieved context:** selected files, docs, tickets, issues, or memory records.
- <InlineEmojiImage asset={EMOJI.operator} size={18} /> **Current user request:** the active task the model is supposed to solve now.
...
<ScrollDrivenFigure caption="Each turn rebuilds the context window. The layer view is only a source map; tool calls and tool results matter after the harness serializes them into raw context for a later request.">
...
<ScrollDrivenFigure caption="Interactive harness agents hand more autonomy to the LLM; the tradeoff is higher operator skill in scope, review, and verification.">
...
<ScrollDrivenFigure caption="Structured control-plane agents make orchestration the primary actor. The LLM is a bounded subroutine called, validated, retried, audited, and stopped by deterministic code.">
...
**Next:** [Lesson 3: Prompting 101](../methodology/lesson-3-prompting-101.mdx)
