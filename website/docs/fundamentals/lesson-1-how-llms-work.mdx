...
<figure>
  ...
  <figcaption>Each new token is predicted from <em>all</em> previous tokens in the context window — the sequence grows one token at a time, feeding back as input for the next prediction.</figcaption>
</figure>
