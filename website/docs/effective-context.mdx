...
<DiagramFrame kicker="LLM mechanics" title="Effective context is smaller than advertised context" size="full" narration="The number on the box is not the number you get. On one independent retrieval test, every model's recall falls as the window grows, and the shape of the fall is the real spec. Qwen 3.8 Max scores 92.3 percent at 128K, 67.7 percent at 256K, then 29.7 percent at 512K. A curve that stays flat sustains precise retrieval; a cliff marks where context engineering has to keep the task, or where the model belongs on work that does not depend on recall. The advertised window promises neither — it only says how many tokens the model accepts.">
