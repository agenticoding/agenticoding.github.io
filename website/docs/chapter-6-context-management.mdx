...
The core problem is positional. The [lost-in-the-middle effect](/chapter-1-how-llms-work) means the same instruction can be reliable at the top or bottom of the window and unreliable in the middle. A 200K-token window sounds enormous until you realize a single MCP server can consume 55K tokens just for tool schemas — before the agent reads a single line of your code. Capacity is not the goal. Controlled attention is the goal.

This chapter covers the five mechanisms that decide what enters the context window and where: **context files**, **MCP tool schemas**, **skills**, **sub-agents**, and **runtime management**. Each operates at a different layer, with different tradeoffs between token cost and attention reliability.
...
[Chapter 1: LLMs Demystified](/chapter-1-how-llms-work#long-context-is-not-reliable-memory) covers the model-selection lesson: bigger context does not mean better retrieval, and newer models can improve coding or tool use while regressing on long-context precision.

<DiagramFrame kicker="Context management" title="Attention is strongest at the edges" size="standard">
...
This chapter starts from the operational consequence: **context engineering exists because storage is not bandwidth.** The job is not to fit everything. The job is to keep the active task, current constraints, and relevant evidence where the model can still use them.
...
<DiagramFrame kicker="Context management" title="The primacy squeeze pushes tasks downward" size="standard" caption={'The Primacy Squeeze: as AGENTS.md grows, the user task gets pushed from the primacy zone into the forgotten middle.'}>
...
See [Chapter 5](/chapter-5-grounding#solution-2-sub-agents-for-context-isolation) for sub-agent grounding patterns.
...
**Next:** [Chapter 7: Reliability Levers](./chapter-7-reliability-levers.md)
