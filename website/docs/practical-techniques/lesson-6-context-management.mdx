...
[Lesson 1: LLMs Demystified](/fundamentals/lesson-1-how-llms-work#long-context-is-not-reliable-memory) covers the model-selection lesson: bigger context does not mean better retrieval, and newer models can improve coding or tool use while regressing on long-context precision.
...
This lesson starts from the operational consequence: **context engineering exists because storage is not bandwidth.** The job is not to fit everything. The job is to keep the active task, current constraints, and relevant evidence where the model can still use them.
