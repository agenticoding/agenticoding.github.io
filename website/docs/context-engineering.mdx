...
**Context effect:** eager loading places every schema in the fixed request prefix before the model plans its first action. Deferred loading keeps most schemas out initially, then adds task-relevant definitions when discovery expands them. The tradeoff is placement, not just capacity: eager schemas apply pressure on every call, while deferred schemas spend context only after discovery but remain available for the rest of the session.
...
<DiagramFrame
  kicker="Context management"
  title="Discovery costs context; unused schemas cost more"
  size="wide"
  caption={
...
Tool Search can return several matches (up to five), and loaded schemas persist for the rest of the session. A longer task that needs new capabilities can trigger additional searches and expansions; the crossover therefore depends on schema size, result breadth, task breadth, and selection quality.
...
<DiagramFrame
  kicker="Context management"
  title="Skills can be operator-triggered or model-invoked"
  size="wide"
  caption={
    'Skill invocation paths: manual selection is reliable; model matching is automatic but probabilistic.'
  }
>
...
<DiagramFrame
  kicker="Context management"
  title="Sub-agents isolate context then return synthesis"
  size="standard"
  caption={
    'Sub-agent context isolation: research consumed internally, compact synthesis returned.'
  }
>
  <SubAgentDiagram />
</DiagramFrame>
...
<DiagramFrame
  kicker="Context management"
  title="Two sub-agent architectures trade flexibility for reproducibility"
  size="wide"
  caption="Deterministic pipelines are structured and reproducible; autonomous exploration is adaptive but non-deterministic."
>
  <ArchitectureDiagram />
</DiagramFrame>
