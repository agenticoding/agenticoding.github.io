...
import HarnessWorkbench from '@site/src/components/VisualElements/HarnessWorkbench';
...
<DiagramFrame
  ...
  title="Harnesses turn model calls into controlled work"
  size="wide"
  narration="A harness agent hands most next-step decisions to the model. A human sets intent and limits, then the model chooses what to read, search, edit, and test through the tools it's been given, with an approval gate before anything risky lands. That's the simplest architecture to build and the most flexible, which is why it feels like leverage. The cost lands on you: with the model making so many probabilistic choices, variance creeps into search paths, edits, and conclusions, and the wider that decision surface, the more the operator has to know about scoping, steering, reviewing, and verifying the work."
  caption="Harness agents hand more autonomy to the LLM; the tradeoff is higher operator skill in scope, review, and verification."
>
  <HarnessWorkbench />
</DiagramFrame>
...
<DiagramFrame kicker="Agent mechanics" title="Harness design changes agent behavior" size="full" narration="Hold the model constant and change only the harness, and measured coding accuracy still moves a lot. In the benchmark's same-model rows, the best harness reaches 84.7 percent on Terminal-Bench while the weakest trails at 66.1. Same model, same task set, nearly nineteen points of spread, purely from how the surrounding harness is designed. The point isn't which harness wins, it's that the harness is a first-class variable in your results, not plumbing. If you're measuring a model without fixing the harness, you're not really measuring the model.">
...
**Next:** [Workflow Agents](./workflow-agents.mdx)
