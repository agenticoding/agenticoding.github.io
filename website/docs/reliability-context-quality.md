...
Most providers charge a per-token surcharge past a context threshold — often double the input rate, well inside the advertised window. For one-shot retrieval tasks that premium buys tokens in the model's strong accuracy zone; for agentic coding it buys tokens in the forgotten middle, where attention is weakest and requirements go to die. Retrieval keeps you under the threshold; feeding everything in lives above it.
:::
