...
const invalidSvgHeightAttribute =
  /<svg\b[\s\S]*?\bheight\s*=\s*["']auto["'][\s\S]*?>/;
const invalidSvgHeightStyle =
  /<svg\b[\s\S]*?\bstyle\s*=\s*\{\{[\s\S]*?\bheight\s*:\s*["']auto["'][\s\S]*?\}\}[\s\S]*?>/;
...
test('svg elements do not use invalid auto height values', () => {
  const violations = collectTsxFiles(componentsDir).filter((path) => {
    ...
    return (
      invalidSvgHeightAttribute.test(source) ||
      invalidSvgHeightStyle.test(source)
    );
  });
