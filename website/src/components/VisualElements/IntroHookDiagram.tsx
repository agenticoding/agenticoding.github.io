...
export default function IntroHookDiagram() {
  ...
  const fan1T = useStrokeDraw(
    fan1Ref as React.RefObject<SVGGeometryElement | null>,
    phase,
    0.8,
    0.995
  );
  const fan2T = useStrokeDraw(
    fan2Ref as React.RefObject<SVGGeometryElement | null>,
    phase,
    0.84,
    0.995
  );
  const fan3T = useStrokeDraw(
    fan3Ref as React.RefObject<SVGGeometryElement | null>,
    phase,
    0.88,
    0.995
  );
  ...
  const mainPromptPos = (() => {
    ...
    const t = Math.min((phase - 0.6) / 0.2, 1); // 0.6→0.80 maps to 0→1
  ...
  return (
    ...
    >
      <defs>
        ...
        {FAN_ARCS.map((fan) => (
          <path key={fan.pathDefId} id={fan.pathDefId} d={fan.d} />
        ))}
      </defs>
      ...
      {FAN_ARCS.map((fan, i) => (
        <Ghost
          ...
          reached={dispatched}
          style={
            dispatched ? { transitionDelay: `${fan.nodeDelay}ms` } : undefined
          }
        />
      ))}
      ...
      <g
        className={clsx(
          styles.actorNode,
          wasReached('operator') && shared.actEntered
        )}
      >
      ...
      {mainPromptPos && (
        <g
          transform={`translate(${mainPromptPos.x}, ${mainPromptPos.y})`}
          style={{ opacity: mainPromptPos.opacity }}
        >
      ...
      {FAN_ARCS.map((fan, i) => (
        <g
          ...
          className={clsx(
            shared.workerNode,
            dispatched && shared.workerEntered
          )}
          style={{
            transitionDelay: dispatched ? `${fan.nodeDelay}ms` : undefined,
          }}
        >
      ...
      <g
        className={clsx(
          styles.labels,
          wasReached('labels') && shared.actEntered
        )}
      >
        <text
          x={90}
      ...
      {FAN_ARCS.map((fan, i) => (
        <text
          ...
          className={clsx(
            shared.workerLabel,
            dispatched && shared.workerLabelEntered
          )}
          fill="var(--visual-neutral)"
          style={{
            fontFamily: 'var(--font-mono)',
            transitionDelay: dispatched
              ? `${fan.nodeDelay + 100}ms`
              : undefined,
          }}
          fontSize={9}
          fontWeight={400}
