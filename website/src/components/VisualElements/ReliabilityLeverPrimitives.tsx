...
export function TextLine({
  ...
  return (
    <text
      ...
      stroke={halo ? 'var(--surface-page)' : undefined}
      strokeWidth={halo ? 'var(--stroke-heavy)' : undefined}
      strokeLinecap={halo ? 'round' : undefined}
...
export function Card({
  ...
  return (
    <g>
      ...
      {lines.map((line, i) => (
        <TextLine key={line} x={x + G * 4} y={y + G * (7 + i * 2.5)}>
          {line}
        </TextLine>
      ))}
    </g>
...
export function Arrow({
  ...
  if (angular)
    return (
      <polyline
        points={`${x1},${y1} ${(x1 + x2) / 2},${y1} ${(x1 + x2) / 2},${y2} ${x2},${y2}`}
        fill="none"
        stroke={tone}
        strokeWidth="var(--stroke-heavy)"
