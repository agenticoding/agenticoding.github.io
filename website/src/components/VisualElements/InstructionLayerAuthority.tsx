...
const CASES: RoleCase[] = [
  {
    role: 'SYSTEM',
    behavior: 'standing rule',
    detail: 'enforce JSON shape',
    tone: 'cyan',
    emoji: 'compass',
    y: 44,
    mobileY: 156,
  ...
  {
    role: 'USER',
    behavior: 'task preference',
    detail: 'follow unless overridden',
    tone: 'neutral',
    emoji: 'operator',
    y: 118,
    mobileY: 210,
  ...
  {
    role: 'OBSERVATION',
    behavior: 'observed text',
    detail: 'data, not command',
    tone: 'indigo',
    emoji: 'receipt',
    y: 192,
    mobileY: 264,
...
const gearCycleStyle: React.CSSProperties = {
  animationDuration: `${ROLE_SLOT_MS}ms`,
};
...
function PromptSlide({ x, y }: { x: number; y: number }) {
  return (
    <g>
      ...
      <text x={x + 16} y={y + 24} className={styles.slideLabel}>
        same instruction text
      </text>
      {PROMPT_LINES.map((line, index) => (
        <PromptLine key={line} x={x + 16} y={y + 45 + index * 18} line={line} />
      ))}
    </g>
  );
}
  ...
  return (
    <g>
      <rect
        x={x - 5}
        y={y - 12}
        width={line.length * 7.1 + 10}
        height={17}
        className={styles.promptChip}
      />
      <text x={x} y={y} className={styles.promptText}>
        {line}
      </text>
    </g>
  );
...
function LayerFilter({
  ...
  return (
    <g
      className={styles.filterStory}
      style={storyStyle(index, FILTER_KEYFRAMES)}
    >
      <rect
        x={x}
        y={y}
        width={142}
        height={46}
        className={clsx(styles.filterPlate, toneClass(item.tone))}
      />
      <EmojiImage asset={EMOJI[item.emoji]} x={x + 10} y={y + 9} size={28} />
      <text x={x + 48} y={y + 19} className={styles.filterRole}>
        {item.role}
      </text>
      <text x={x + 48} y={y + 33} className={styles.filterLabel}>
        source role
      </text>
    </g>
  );
...
function ProjectionPanel({
  ...
  return (
    <g
      className={styles.projectionStory}
      style={storyStyle(index, PROJECTION_KEYFRAMES)}
    >
      <rect
        x={x}
        y={y}
        width={width}
        height={52}
        className={clsx(styles.projectionPanel, toneClass(item.tone))}
      />
      <text x={x + 14} y={y + 20} className={styles.behaviorText}>
        {item.behavior}
      </text>
      <text x={x + 14} y={y + 37} className={styles.detailText}>
        {item.detail}
      </text>
    </g>
  );
...
function RoleFlow({
  ...
  return (
    <>
      <g
        className={styles.flowStory}
        style={storyStyle(index, FILTER_KEYFRAMES)}
      >
        <path
          d={promptPath}
          className={clsx(styles.rolePath, toneClass(item.tone))}
        />
        <path
          d={filterPath}
          className={clsx(styles.rolePath, toneClass(item.tone))}
        />
        <polygon
          points={rightTipPoints(FILTER_LEFT_X, cy)}
          className={clsx(styles.roleTip, toneClass(item.tone))}
        />
        <polygon
          points={rightTipPoints(PROJECTOR_LEFT_X, FLOW_CY)}
          className={clsx(styles.roleTip, toneClass(item.tone))}
        />
      </g>
      <g
      ...
      >
        ...
        />
        <polygon
          points={rightTipPoints(PROJECTION_LEFT_X, cy)}
          className={clsx(styles.roleTip, toneClass(item.tone))}
        />
      </g>
    </>
...
function DesktopDiagram() {
  return (
    <svg
      ...
      width="100%"
      role="img"
      aria-label="The same instruction text enters different source roles before the LLM predicts different behavior."
      className={clsx(styles.diagram, styles.desktop)}
      style={diagramTimingStyle}
      xmlns="http://www.w3.org/2000/svg"
    >
      <text x={145} y={34} textAnchor="middle" className={styles.columnLabel}>
        same words
      </text>
      <text x={397} y={34} textAnchor="middle" className={styles.columnLabel}>
        source role
      </text>
      <text x={651} y={34} textAnchor="middle" className={styles.columnLabel}>
        model prediction
      </text>
      <text x={833} y={34} textAnchor="middle" className={styles.columnLabel}>
        behavior
      </text>
      <PromptSlide x={40} y={90} />
      {CASES.map((item, index) => (
        <DesktopCase key={item.role} item={item} index={index} />
      ))}
      <GearNode
        x={620}
        y={110}
        size={62}
        className={clsx(
          styles.projectorGear,
          'inference-llm-cycle idle-inference-cycle'
        )}
        style={gearCycleStyle}
      />
    </svg>
  );
}
...
function MobileDiagram() {
  return (
    <svg
      ...
      width="100%"
      role="img"
      aria-label="Mobile view: same instruction text, source role, model prediction, different behavior."
    ...
    >
      ...
      <text x={180} y={145} textAnchor="middle" className={styles.columnLabel}>
        source role
      </text>
      {CASES.map((item, index) => (
        <MobileCase key={item.role} item={item} index={index} />
      ))}
      ...
      <text x={180} y={437} textAnchor="middle" className={styles.columnLabel}>
        behavior
      </text>
    </svg>
  );
