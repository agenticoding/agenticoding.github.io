...
export default function SequenceDiagram({ columns, rows, ariaLabel }: Props) {
  ...
  const colSpacing =
    nc > 1
      ...
      : viewW - 2 * PAD_H;
  const padH =
    nc > 1 ? (viewW - COL_ICON_SIZE - (nc - 1) * colSpacing) / 2 : PAD_H;
  const colCenter = (i: number) =>
    nc === 1 ? padH + (viewW - 2 * padH) / 2 : padH + nodeHalf + i * colSpacing;

  const colX = (id: string) => {
    const i = columns.findIndex((c) => c.id === id);
    if (i < 0) {
      console.error(`SequenceDiagram: unknown column id "${id}"`);
      return 0;
    }
    return colCenter(i);
  };
    ...
    <div ref={containerRef} className={styles.wrapper}>
      ...
      <div
        className={clsx(
          styles.stickyHeader,
          styles.structure,
          structureReached && styles.structureIn
        )}
      ...
      >
        ...
        {rows.map((row, i) => {
          const y = rowY(i);
          const rowReached = getSequenceRowReached(
            mounted,
            phase,
            i,
            rows.length
          );
          const rowGhostClass = ghostClass(
            mounted,
            getSequenceMessageGhostReached(mounted, phase, i, rows.length)
          );
          ...
          if (row.type === 'note') {
            ...
            if (colIdx < 0) {
              console.error(`SequenceDiagram: unknown column id "${row.on}"`);
              return null;
            }
          ...
          if (x1 === x2) {
            console.error(
              `SequenceDiagram: self-message "${row.label}" (from === to: "${row.from}") is not supported`
            );
            return null;
          }
          const midX = (x1 + x2) / 2;
