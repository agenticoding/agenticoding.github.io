...
const DESKTOP_STEPS = [
  ...
  },
  {
    ...
    y: 32,
    title: 'Evidence',
    detail: 'correlated checks',
    tone: 'violet',
  },
  {
    x: 552,
    y: 200,
    title: 'Analyze + retest',
    detail: 'reproduce failures',
    tone: 'warning',
  },
  {
    x: 288,
    y: 200,
    title: 'Bounded release',
    detail: 'canary + rollback',
    tone: 'success',
  },
  {
    x: 24,
...
function DesktopDiagram({ markerPrefix }: { markerPrefix: string }) {
  return (
    <svg
      className={styles.validationDesktop}
      viewBox="0 0 792 352"
      role="img"
      aria-label="Validation lifecycle from a product claim through representative operating conditions, correlated evidence, retesting, bounded release, and field signals that update the profile."
    >
      <DiagramArrowMarkers
        prefix={markerPrefix}
        tones={['neutral', 'indigo', 'violet', 'warning', 'success', 'cyan']}
      />
      <DesktopArrows markerPrefix={markerPrefix} />
      {DESKTOP_STEPS.map((step) => (
        <Tile key={step.title} {...step} />
      ))}
    </svg>
  );
}
...
function MobileDiagram({ markerPrefix }: { markerPrefix: string }) {
  return (
    <svg
      className={styles.validationMobile}
      viewBox="0 0 320 672"
      role="img"
      aria-label="Validation lifecycle from a product claim through representative operating conditions, correlated evidence, retesting, bounded release, and field signals that update the profile."
    >
      <DiagramArrowMarkers
        prefix={`${markerPrefix}-mobile`}
        tones={['neutral', 'indigo', 'violet', 'warning', 'success', 'cyan']}
      />
      <MobileArrows markerPrefix={`${markerPrefix}-mobile`} />
      {MOBILE_STEPS.map((step) => (
        <Tile key={step.title} x={24} {...step} />
      ))}
    </svg>
  );
}
...
function Tile({
  ...
  title: string;
  detail: string;
  tone: 'neutral' | 'indigo' | 'violet' | 'warning' | 'success' | 'cyan';
}) {
  return (
    <DiagramTile
      x={x}
...
function DesktopArrows({ markerPrefix }: { markerPrefix: string }) {
  return (
    <>
      ...
      />
      <DiagramArrow
        d="M 132 200 C 132 152 184 152 288 112"
        markerIdPrefix={markerPrefix}
        tone="cyan"
      />
    </>
...
function MobileArrows({ markerPrefix }: { markerPrefix: string }) {
  return (
    <>
      {MOBILE_STEPS.slice(0, -1).map((step, index) => (
        <DiagramArrow
          key={step.title}
          d={`M 160 ${step.y + 80} V ${MOBILE_STEPS[index + 1].y}`}
          markerIdPrefix={markerPrefix}
          tone={MOBILE_STEPS[index + 1].tone}
        />
      ))}
      <DiagramArrow
        d="M 24 584 H 8 V 168 H 24"
        markerIdPrefix={markerPrefix}
        tone="cyan"
      />
    </>
