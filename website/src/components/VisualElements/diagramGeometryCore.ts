import { DIAGRAM_MARKER, DIAGRAM_STROKE } from './diagramScale.ts';
...
function trimPoint(end: Point, from: Point, amount: number): Point {
  ...
  const trim = Math.min(amount, length * MAX_TERMINAL_TRIM_RATIO);
...
export function trimPathEnd(
  ...
  if (horizontal) {
    ...
    const endX = Number(x);
    const distance = Math.abs(endX - fromX);
    const trim = Math.min(amount, distance * MAX_TERMINAL_TRIM_RATIO);
    return `${prefix}${coord(endX - Math.sign(endX - fromX) * trim)}`;
  }
  ...
  if (vertical) {
    ...
    const endY = Number(y);
    const distance = Math.abs(endY - fromY);
    const trim = Math.min(amount, distance * MAX_TERMINAL_TRIM_RATIO);
    return `${prefix}${coord(endY - Math.sign(endY - fromY) * trim)}`;
  }
